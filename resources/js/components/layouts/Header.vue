<template>
  <nav class="bg-primary text-white shadow-lg relative">
    <div class="container mx-auto px-4 py-3 flex justify-between items-center relative">
      <Link href="/" class="flex items-center space-x-2 hover:opacity-80 transition cursor-pointer">
        <img
          v-if="siteLogo"
          :src="siteLogo"
          :alt="siteName"
        ...
        <span v-else-if="!siteLogo" class="text-xl font-bold">{{ siteName }}</span>
      </Link>
      <div class="hidden md:flex space-x-6 items-center">
        ...
        <div v-if="authStore.isAuthenticated" class="relative user-dropdown">
          ...
          >
            ...
            />
            <span class="font-medium hidden lg:block">{{ authStore.currentUser?.name }}</span>
            <ChevronDownIcon
            ...
            >
              <div class="px-4 py-2 border-b border-gray-200">
                <p class="font-semibold text-gray-900 break-words">{{ authStore.currentUser?.name }}</p>
                <p class="text-sm text-gray-500 break-words">{{ authStore.currentUser?.email }}</p>
              </div>
            <Link
              v-if="isManager"
              href="/admin"
      ...
      <div class="md:hidden relative">
        <button
          ...
          type="button"
        >
          <Bars3Icon class="w-6 h-6 transition-transform" :class="menuOpen ? 'rotate-90' : 'rotate-0'" />
        </button>
        ...
        >
          ...
          <div class="bg-primary text-white p-4 flex items-center justify-between">
            <div class="flex items-center space-x-2">
              ...
              <h2 v-else-if="!siteLogo" class="text-lg font-bold">{{ siteName }}</h2>
            </div>
            <button
              @click="menuOpen = false"
          ...
          <div class="p-4">
            ...
            <div v-if="authStore.isAuthenticated" class="space-y-4">
              ...
              <div class="space-y-2">
                ...
                >
                  <UserIcon class="w-5 h-5 text-primary" />
                  <span class="font-medium">Thông tin của tôi</span>
                </Link>
                <Link
                  href="/my-rentals"
                  @click="menuOpen = false"
              ...
              <div class="pt-4 border-t border-gray-200">
                <button
                  @click="handleLogout"
                  :class="['menu-item flex items-center space-x-3 w-full py-3 px-4 text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200', { 'menu-item-animated': menuOpen }]"
                  :style="{ '--delay': '0.3s' }"
                >
                  <ArrowRightOnRectangleIcon class="w-5 h-5" />
...
const handleAvatarError = (event) => {
  ...
  if (user?.name) {
    ...
    event.target.src = 'https://ui-avatars.com/api/?name=User&background=000066&color=fff&size=128'
  }
}
</script>
