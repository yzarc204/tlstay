  ...
  >
    <div class="max-w-md w-full">
      ...
      <div class="text-center mb-8">
        ...
        <h2 class="text-3xl font-bold text-white mb-2">Tạo tài khoản</h2>
        <p class="text-white/80">Tham gia TL STAY ngay hôm nay</p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8">
        <form @submit.prevent="handleRegister">
          ...
          <div class="mb-4">
            ...
            <p class="mt-1 text-xs text-gray-500">Ví dụ: 0123456789 hoặc +84123456789</p>
          </div>
          ...
          <div
            v-if="successMessage"
            class="mb-4 p-3 bg-primary-50 border border-primary-200 rounded-lg"
          >
        ...
        <p class="mt-6 text-center text-sm text-gray-600">
          Đã có tài khoản?
          <Link href="/login" class="font-medium text-primary hover:text-secondary">
            Đăng nhập
          </Link>
        </p>
      </div>
...
const handleRegister = async () => {
  ...
  if (!phoneRegex.test(cleanPhone)) {
    errorMessage.value = 'Số điện thoại không hợp lệ (ví dụ: 0123456789 hoặc +84123456789)'
    return
  }
  ...
  authStore.register(
    ...
    // onError
    (message) => {
      errorMessage.value = message || 'Đã có lỗi xảy ra khi đăng ký'
      loading.value = false
    }
  )
}
