    ...
    <div class="booking-detail bg-light min-h-screen py-12">
      <div class="container mx-auto px-4">
        ...
        <div class="mb-6">
          ...
          <h1 class="text-3xl font-bold text-secondary">Chi tiết đặt phòng</h1>
        </div>
        ...
        <div v-else class="grid lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card p-6">
              <div class="flex items-start justify-between mb-6">
                <div>
                  <h2 class="text-2xl font-bold text-secondary mb-2">Thông tin đặt phòng</h2>
                  <p class="text-gray-600">Mã đặt phòng: <span class="font-mono font-semibold text-primary">{{ booking.booking_code }}</span></p>
                </div>
                <span
                  class="badge text-sm"
            ...
            <!-- Payment Info Card -->
            <div class="card p-6">
              <h2 class="text-2xl font-bold text-secondary mb-6">Thông tin thanh toán</h2>
            ...
            <!-- Contract Card -->
            <div class="card p-6">
              <h2 class="text-2xl font-bold text-secondary mb-6">Hợp đồng</h2>
              ...
              <div class="space-y-4">
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <div class="flex space-x-2">
                    ...
                    <Link
                      v-if="!booking.contract_signed && booking.payment_status === 'paid'"
            ...
            <!-- Invoices Card -->
            <div class="card p-6">
              <h2 class="text-2xl font-bold text-secondary mb-6">Hóa đơn dịch vụ</h2>

              <div v-if="booking.invoices && booking.invoices.length > 0" class="space-y-4">
                <div
                ...
                >
                  ...
                  <div class="grid md:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p class="font-semibold text-gray-900">{{ formatPrice(invoice.electricity_amount) }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-gray-600">Nước</p>
                      <p class="font-semibold text-gray-900">{{ formatPrice(invoice.water_amount) }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-gray-600">Internet</p>
                      <p class="font-semibold text-gray-900">{{ formatPrice(invoice.other_fees) }}</p>
                    </div>
                    <div>
                      <p class="text-sm text-gray-600">Tổng cộng</p>
                      <p class="font-bold text-primary text-lg">{{ formatPrice(invoice.total) }}</p>
                    </div>
                  </div>

                  <div class="flex items-center justify-between pt-3 border-t">
                    <div class="text-sm text-gray-600">
                      <p v-if="invoice.due_date">Hạn thanh toán: {{ formatDate(invoice.due_date) }}</p>
                      <p v-if="invoice.paid_at">Đã thanh toán: {{ formatDateTime(invoice.paid_at) }}</p>
                    </div>
                    <div class="flex space-x-2">
                      <Link
              ...
              <div v-else class="text-center py-8 text-gray-500">
                <DocumentTextIcon class="w-16 h-16 mx-auto mb-2 text-gray-300" />
                <p>Chưa có hóa đơn dịch vụ</p>
              </div>
            </div>

            <!-- Review Card -->
            <div v-if="booking.review || booking.can_review" class="card p-6">
              <h2 class="text-2xl font-bold text-secondary mb-6">Đánh giá</h2>

              <!-- Review Form -->
              <ReviewForm
                v-if="booking.can_review && booking.payment_status === 'paid'"
          ...
          <div class="lg:col-span-1">
            <div class="card p-6 sticky top-24 space-y-4">
              <h3 class="text-lg font-bold text-secondary mb-4">Thao tác</h3>
              ...
              <!-- Contract Actions -->
              <div v-if="booking.payment_status === 'paid'" class="space-y-3">
                <a
                  :href="`/contract/${booking.id}/preview`"
                  target="_blank"
              ...
              <div class="pt-6 border-t">
                ...
                <div v-if="booking.owner" class="bg-primary-50 rounded-lg p-4">
                  ...
                  <div class="space-y-2 text-sm">
                    ...
                    <p v-if="booking.owner.phone" class="flex items-center text-gray-700">
                      <PhoneIcon class="w-4 h-4 mr-2 text-primary" />
                      <a :href="`tel:${booking.owner.phone}`" class="hover:text-primary">
                        {{ booking.owner.phone }}
                      </a>
                    </p>
                    <p v-if="booking.owner.email" class="flex items-center text-gray-700">
                      <EnvelopeIcon class="w-4 h-4 mr-2 text-primary" />
                      <a :href="`mailto:${booking.owner.email}`" class="hover:text-primary break-all">
                        {{ booking.owner.email }}
                      </a>
                    </p>
                  </div>
              ...
              <!-- Map -->
              <div v-if="booking.house.latitude && booking.house.longitude" class="pt-6 border-t">
                <h4 class="font-semibold text-gray-800 mb-3">Vị trí</h4>
